...
def trim_silence(audio, silence_threshold_db=-40, chunk_size_ms=10, tail_padding_ms=150):
    """
    ...
    Detects the full extent of trailing silence, then subtracts tail_padding_ms
    from that amount before trimming. This preserves a natural-sounding tail from
    the original audio rather than cutting to the speech boundary and re-adding
    synthetic silence. If the trailing silence is shorter than the padding window,
    nothing is trimmed.
    ...
    Args:
        ...
        tail_padding_ms (int): Milliseconds of trailing silence to retain as a
                               safety window (default: 750ms). Only silence
                               beyond this amount is removed.

    Returns:
        AudioSegment: Trimmed audio segment with natural tail preserved
    """
    # Detect total trailing silence
    end_trim_ms = detect_leading_silence(audio.reverse(), silence_threshold=silence_threshold_db,
                                         chunk_size=chunk_size_ms)

    # Only trim the silence that exceeds the padding window — keeps tail_padding_ms of natural silence
    actual_trim_ms = max(0, end_trim_ms - tail_padding_ms)

    duration_ms = len(audio)
    trimmed = audio[:duration_ms - actual_trim_ms]

    trimmed_end = actual_trim_ms / 1000
    trimmed_duration = len(trimmed) / 1000
    original_duration = duration_ms / 1000

    print(f"  Silence trimmed — end: {trimmed_end:.2f}s removed ({tail_padding_ms}ms safety window kept) "
          f"| {original_duration:.2f}s → {trimmed_duration:.2f}s")

    return trimmed


def mp3_to_wav(mp3_path, wav_path=None, trim=True, silence_threshold_db=-40, tail_padding_ms=150):
    """
    ...
    Args:
        ...
        tail_padding_ms (int): Milliseconds of silence to restore after trimming
                               (default: 750ms). Only used when trim=True.
...
def batch_convert_mp3_to_wav(input_dir, output_dir, trim=True, silence_threshold_db=-40, tail_padding_ms=150):
    """
